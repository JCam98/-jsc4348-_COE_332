...
import petname, random, datetime, uuid, redis, flask, json

# Create a client connection for the redis server:

rd=redis.StrictRedis(host='jcam98-redis-midterm', port=6379, db=0)
...
def get_updated_creature():
     ...
     for i in range(0,animal_num):
         ...
         animal_list_binary =[] # Initialize list to store animal with UUID parameter
...
def get_avg_leg_num():
     ...
     for i in range(0,animal_num):
         ...
         leg_num_list_str.append(rd.hget(animal_labels[i],'legs').decode('utf-8'))
